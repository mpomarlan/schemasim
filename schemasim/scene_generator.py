...
def interpretScene(schemas, simulator, simulate_counterfactuals=True, render=False, nframes=250, sceneFolder=None):
    ...
    retq["scene_results"]["default"] = checkSceneExpectations(enet.schemas(), simulator, os.path.join(sceneFolder, "animation.log"), condition=Default())
    if simulate_counterfactuals:
        ...
        for s in enet.schemas():
            if isinstance(s, st.ParameterizedSchema):
                ...
                retq["scene_results"]["counterfactuals"][cId] = checkSceneExpectations(enet.schemas(), simulator, os.path.join(cFolder, "animation.log"), condition=CollisionDisabled(obj=s))
